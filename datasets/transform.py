...
## 这个类负责收集数据字典中指定的键值。通过传入的 keys 参数，它会从数据字典中选取相应的数据。此外，offset_keys_dict 参数用于记录数据集中每个键对应的偏移量。
...
@TRANSFORMS.register_module()
## Copy 类用于复制数据。它允许从数据字典中复制特定键的值到新的键中。这在数据增强时，保留原始数据的同时进行修改操作非常有用。
class Copy(object):
...
@TRANSFORMS.register_module()
## 这个类的功能是将不同类型的数据转换为PyTorch张量（tensor）。这是准备数据输入到神经网络模型中的一个常见步骤。它能处理各种数据类型，包括整数、浮点数、NumPy数组等。
...
#         return data_dict
## NormalizeColor 类将颜色数据标准化到 [−0.5,0.5] 范围内。
# class NormalizeColor(object):
#     def __call__(self, data_dict):
#         if "strength" in data_dict.keys():
#             data_dict["strength"] = data_dict["strength"] / 255 - 0.5  # 调整范围到 [-0.5, 0.5]
#         return data_dict
class NormalizeColor(object):
    def __call__(self, data_dict):
        if "strength" in data_dict.keys():
            strength = data_dict["strength"]
            min_val = strength.min()  # 获取最小值
            max_val = strength.max()  # 获取最大值
            data_dict["strength"] = (strength - min_val) / (max_val - min_val) - 0.5  # 归一化到 [0, 1]
        return data_dict
...
@TRANSFORMS.register_module()
## NormalizeCoord 类用于将坐标数据标准化，即去中心化并缩放，使得数据点集中在原点附近，大小规范化。
...
@TRANSFORMS.register_module()
## 这个类对点云坐标进行正向偏移，使所有坐标点变为正值。这通常用于确保数据处理中没有负坐标值的问题，特别是在需要将数据放入某些算法或数据结构中时很有用。
...
@TRANSFORMS.register_module()
## CenterShift 类用于将坐标数据平移，使得点云数据中心化。
...
@TRANSFORMS.register_module()
## RandomShift 类则随机平移点云数据，增加数据的多样性。
...
@TRANSFORMS.register_module()
## 这个类将坐标数据限制在一个预设的范围内，通常是为了去除离群点或者根据实际应用需求裁剪数据。
...
@TRANSFORMS.register_module()
## 模拟数据丢失的情况，随机地从点云中删除一部分点。这是一种常见的数据增强技术，用于提高模型对不完整数据的鲁棒性。
...
class RandomDropout(object):
    ...
    def __call__(self, data_dict):
        if random.random() < self.dropout_application_ratio:
            n = len(data_dict["coord"])
            num_to_keep = int(n * (1 - self.dropout_ratio))
            idx = np.random.choice(n, num_to_keep, replace=False)
            max_index = data_dict["strength"].shape[0] - 1
            safe_idx = np.clip(idx, 0, max_index)
            if "coord" in data_dict.keys():
                data_dict["coord"] = data_dict["coord"][safe_idx]
            if "strength" in data_dict.keys():
                data_dict["strength"] = data_dict["strength"][safe_idx]
            if "segment" in data_dict.keys():
                data_dict["segment"] = data_dict["segment"][safe_idx]
            if "instance" in data_dict.keys():
                data_dict["instance"] = data_dict["instance"][safe_idx]
            # if "room_FPFH1" in data_dict.keys():
            #     data_dict["room_FPFH1"] = data_dict["room_FPFH1"][safe_idx]
            # if "room_FPFH2" in data_dict.keys():
            #     data_dict["room_FPFH2"] = data_dict["room_FPFH2"][safe_idx]
            # if "room_FPFH3" in data_dict.keys():
            #     data_dict["room_FPFH3"] = data_dict["room_FPFH3"][safe_idx]
            if "fpfh1" in data_dict.keys():
                data_dict["fpfh1"] = data_dict["fpfh1"][safe_idx]
            if "fpfh2" in data_dict.keys():
                data_dict["fpfh2"] = data_dict["fpfh2"][safe_idx]
            if "fpfh3" in data_dict.keys():
                data_dict["fpfh3"] = data_dict["fpfh3"][safe_idx]

        # print(f"After RandomDropout: {data_dict.keys()}")  # Debugging statement
        return data_dict


@TRANSFORMS.register_module()
## 这个类提供了点云旋转的功能。RandomRotate 提供随机角度的旋转
...
@TRANSFORMS.register_module()
## RandomRotateTargetAngle 允许围绕预定义的角度旋转，增加特定方向的数据变化。
...
@TRANSFORMS.register_module()
## 对点云进行随机缩放，改变点云的尺寸，是另一种常见的数据增强方式。
...
#         return data_dict
## 这些类用于对点云的颜色信息进行操作，包括自动对比度调整、颜色平移和添加颜色噪声，以此来增加颜色的变化和模拟实际环境中的视觉效果。
class ChromaticAutoContrast(object):
    def __init__(self, p=0.2, blend_factor=None):
        self.p = p
        self.blend_factor = blend_factor
...
#             data_dict["color"] = np.clip(tr + data_dict["color"], 0, 255)  # 适当处理单通道
#         return data_dict
## 这些类用于对点云的颜色信息进行操作，包括自动对比度调整、颜色平移和添加颜色噪声，以此来增加颜色的变化和模拟实际环境中的视觉效果。
class ChromaticTranslation(object):
    def __init__(self, p=0.95, ratio=0.05):
...
@TRANSFORMS.register_module()
## 将彩色点云随机转换为灰度，这可以帮助模型学习从结构信息中进行推理，而不是依赖于颜色。
...
@TRANSFORMS.register_module()
## 对点云的颜色进行随机调整，包括亮度、对比度、饱和度和色调。这有助于模型在不同光照和色彩条件下保持稳定性。
...
@TRANSFORMS.register_module()
## HueSaturationTranslation 类是专门设计来调整点云中颜色的色调（hue）和饱和度（saturation）
...
@TRANSFORMS.register_module()
## 此类用于随机减弱点云中颜色的强度。通过乘以一个小于1的系数，可以减少颜色的饱和度，模拟不同光照条件下颜色的变化，增强模型对光照变化的鲁棒性。
...
@TRANSFORMS.register_module()
## 这个类实现了弹性扭曲变换，通过在点云上添加随机扰动模拟实际世界中物体的弹性变形。这种变换可以帮助模型学习在物理世界中对象可能的变形。
...
@TRANSFORMS.register_module()
## 将点云数据基于网格进行下采样，这是处理大规模点云数据时常用的技术，可以显著减少计算量。
class GridSample(object):
    def __init__(
        ...
        keys=("coord", "strength",  "segment"),
            # keys=("coord", "strength", "fpfh1", "fpfh2", "fpfh3", "segment"),
...
@TRANSFORMS.register_module()
## SphereCrop 类通过在点云中心创建一个球形区域并且只保留该区域内的点来裁剪点云。这可以用于集中模型的注意力在点云的局部特征上，常用于处理非常大的点云数据集。
        ...
        if self.mode == "all":
            ...
            if total_points > point_max:
                ...
                while idx_uni.size != data_dict["index"].shape[0]:
                    ...
                    idx_crop = np.argsort(dist2)[:point_max]
                    data_crop_dict = dict()
                    for key in ['coord', 'grid_coord', 'strength',  'displacement']:
                        # for key in ['coord', 'grid_coord', 'strength', 'fpfh1', 'fpfh2', 'fpfh3', 'displacement']:
                        if key in data_dict:
            ...
            if total_points > point_max:
                ...
                for key in ['coord', 'origin_coord', 'grid_coord', 'strength', 'segment', 'instance', 'displacement']:
                    # for key in ['coord', 'origin_coord', 'grid_coord', 'strength', 'fpfh1', 'fpfh2', 'fpfh3', 'segment',
...
@TRANSFORMS.register_module()
## 这个类将点云中的点进行随机洗牌。在处理顺序敏感的模型（如某些类型的神经网络）时，打乱输入数据的顺序可以是一个重要的预处理步骤，有助于模型学习到更加通用的特征，而不是依赖于数据的特定排列方式。
class ShufflePoint(object):
    def __call__(self, data_dict):
        ...
        if "strength" in data_dict.keys():
            data_dict["strength"] = data_dict["strength"][shuffle_index]
        # if "room_FPFH1" in data_dict.keys():
        #     data_dict["room_FPFH1"] = data_dict["room_FPFH1"][shuffle_index]
        # if "room_FPFH2" in data_dict.keys():
        #     data_dict["room_FPFH2"] = data_dict["room_FPFH2"][shuffle_index]
        # if "room_FPFH3" in data_dict.keys():
        #     data_dict["room_FPFH3"] = data_dict["room_FPFH3"][shuffle_index]
        if "fpfh1" in data_dict.keys():
            data_dict["fpfh1"] = data_dict["fpfh1"][shuffle_index]
        if "fpfh2" in data_dict.keys():
            data_dict["fpfh2"] = data_dict["fpfh2"][shuffle_index]
        if "fpfh3" in data_dict.keys():
            data_dict["fpfh3"] = data_dict["fpfh3"][shuffle_index]
        # if "normal" in data_dict.keys():
...
@TRANSFORMS.register_module()
## 此类用于剪裁出点云中特定的边界区域。它根据点云中的分割信息来确定哪些点应该保留，哪些点应该剔除，通常用于清理点云数据中的边界点或噪声。
class CropBoundary(object):
    def __call__(self, data_dict):
        ...
        # if "room_FPFH1" in data_dict.keys():
        #     data_dict["room_FPFH1"] = data_dict["room_FPFH1"][mask]
        # if "room_FPFH2" in data_dict.keys():
        #     data_dict["room_FPFH2"] = data_dict["room_FPFH2"][mask]
        # if "room_FPFH3" in data_dict.keys():
        #     data_dict["room_FPFH3"] = data_dict["room_FPFH3"][mask]
        if "fpfh1" in data_dict.keys():
            data_dict["fpfh1"] = data_dict["fpfh1"][mask]
        if "fpfh2" in data_dict.keys():
            data_dict["fpfh2"] = data_dict["fpfh2"][mask]
        if "fpfh3" in data_dict.keys():
            data_dict["fpfh3"] = data_dict["fpfh3"][mask]
        # if "normal" in data_dict.keys():
...
@TRANSFORMS.register_module()
## 该类生成对比视图，常用于对比学习场景，其中需要两个不同的视图来训练模型学习区分不同的数据表示。通过视图转换配置，可以生成具有一定变换的两个数据视图。
class ContrastiveViewsGenerator(object):
    def __init__(
        self,
        # view_keys=("coord", "color", "normal", "origin_coord"),
            view_keys=("coord", "strength",  "origin_coord"),
            # view_keys=("coord", "strength", "fpfh1", "fpfh2", "fpfh3", "origin_coord"),
        view_trans_cfg=None,
    ):
...
@TRANSFORMS.register_module()
## 这个类解析点云中的实例信息，处理实例分割任务中的标签数据。它将标签和实例信息从数据字典中提取出来，进行清理和重新排列，为后续的实例分割任务提供准备。
...
## 这是一个组合类，用于将多个转换组合在一起。它按顺序应用一系列预定义的转换，这在构建复杂的数据预处理管道时非常有用。
